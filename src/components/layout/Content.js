        ...
        <div className="flex-fill container pt-30">
            ...
            <div className={`card p-20 my-30 d-flex flex-column ${styles.contentCard}`}>
                <div className={`d-flex flex-row justify-content-center align-items-center my-30 ${styles.searchbar}`}>
                    ...
                    <input onInput={ handleInput } className="flex-fill" type="text" name="search" id="search" placeholder="Rechercher" />
                </div>
                <div className={styles.grid}>
                    {recipes
                        .filter(r => r.title.toLowerCase().startsWith(filter))
                        .map( ({title, _id, image}) => <Recipe key={_id} title= {title} img= {image} />)}
                </div>
            </div>
        </div>
    )
